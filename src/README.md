...
| `main.py` | Application entrypoint. It prints bring-up status, renders the boot screen when available, shows the configured Wi-Fi SSID on the offline screen when a connection fails, and polls GPIO42 for reconnect or push-to-talk input. |
...
| `lib/button.py` | Active-low GPIO42 side button reader with debounce and press/release events. |
| `lib/io_probe.py` | Explicit GPIO-only probe that cycles status LED states and prints button events. |
| `lib/display.py` | ST7735 display wrapper, PWM backlight control, text layout, and named status screen renderer. |
| `lib/display_probe.py` | Explicit LCD probe that cycles boot, Wi-Fi, ready, recording, processing, speaking, and error screens. |
...
| `lib/assistant_state.py` | Assistant state names and shared LED/display/serial state output mapping. |
| `lib/push_to_talk.py` | Local-only push-to-talk controller that coordinates button events, capture, local service exchange, response display, and playback. |
...
- Normal `main.py` startup is Wi-Fi-tolerant when a local Wi-Fi configuration is
  present: failed initial or button-triggered connections show `OFFLINE` with a
  red LCD status dot and keep GPIO42 polling active. The reconnect press never
  records; after `ONLINE` appears with its green dot, a separate press/release
  pair starts a push-to-talk exchange.
