...
| `feat/02-micropython-skeleton` | Implemented | `src/boot.py`, `src/main.py`, `src/pins.py`, `src/README.md`, and host tests provide safe startup defaults, grouped pin constants, serial-visible bring-up status, protected GPIO10 behavior, and hardware-free regression coverage. `main.py` now continues from safe startup into the local push-to-talk application instead of stopping at a skeleton-ready screen. | Validate the normal boot serial output and push-to-talk readiness on physical hardware. |
| `feat/04-display-bringup` | Implemented, hardware validated | `src/display.py`, `src/display_probe.py`, `src/aipi_lite_config.py`, `src/main.py`, tracked external display drivers under `src/lib/drivers/`, and `tests/test_aipi_lite_display.py` add an ST7735 wrapper, PWM backlight control, named status screens, an opt-in display probe, and host-side layout coverage. Operator hardware validation on 2026-06-25 reported that `display_probe.run_probe(cycles=2)` passed. | Capture a photo or full display probe serial transcript during a future bench run if exact orientation, color, and readability evidence is needed. |
