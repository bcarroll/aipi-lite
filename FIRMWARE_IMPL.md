...
| `tooling/windows-device-validation` | Implemented, hardware validation pending | `install.cmd`, `validate.cmd`, `tools/windows_installer.py`, `tests/test_windows_installer.py`, `README.md`, and `tools/README.md` add application-first Windows uploads, targeted cleanup of legacy root modules moved under `/lib`, and a no-reset physical validation run for display, GPIO status/button, codec, capture, playback, and offline inference probes. Each parsed run records explicit operator observations, preserves redacted local evidence, and creates a new GitHub issue when `gh` is available. | Run `install.cmd --port COMx --yes`, verify the legacy cleanup and offline boot behavior, then run `validate.cmd --port COMx --yes` and review the created issue evidence. |
