...
| `tooling/windows-device-validation` | Implemented, hardware validation pending | `install.cmd`, `install.sh`, `validate.cmd`, `tools/device_application.py`, `tools/windows_installer.py`, host tests, `README.md`, and `tools/README.md` add cache-free application-first Windows uploads to the explicit `mpremote` `:/` device-root destination, shared guarded cleanup of recognized misplaced `/src` trees and legacy root modules moved under `/lib`, cleanup/reset in one `mpremote` connection, a successful manual-power-cycle fallback when only reset confirmation fails, and a validation-only preflight hard reset with a one-second wait before the no-reset physical validation run for display, GPIO status/button, codec, capture, playback, and offline inference probes. The validation probes share one raw-REPL session, report each probe result, and do not reconnect between probes; each parsed run records explicit operator observations, preserves redacted local evidence, and includes up to 12 redacted high-signal upload diagnostics on an application-upload failure before creating a new GitHub issue when `gh` is available. | Run `install.cmd --port COMx --yes`, verify `/main.py` and `/lib`, guarded `/src` removal, offline boot behavior, and reset fallback, then run `validate.cmd --port COMx --yes` and review the created issue evidence. |
...
| `feat/05-local-wifi-policy` | Implemented, hardware validation pending | `src/lib/wifi_config.py`, `src/lib/local_endpoint.py`, `src/lib/wifi_probe.py`, `.gitignore`, `install.sh`, and `tests/test_wifi_policy.py` add ignored local config loading, installer-assisted `local_wifi_config.py` creation, local-only endpoint validation, a Wi-Fi `/health` probe, nonfatal `OFFLINE` UI for connection timeouts, and host-side policy coverage. | Run `wifi_probe.run_probe()` on physical hardware with and without a reachable local network, recording connection, endpoint, offline/online, LED, and display behavior. |
...
| `feat/10-push-to-talk-flow` | Implemented, hardware validation pending | `src/main.py`, `src/lib/assistant_state.py`, `src/lib/push_to_talk.py`, `src/lib/display.py`, `tests/test_main_startup.py`, and `tests/test_push_to_talk_flow.py` add normal-boot push-to-talk startup, an explicit offline state that still enters the GPIO42 poll loop, manual reconnect-on-press that requires a second press to record, LCD `OFFLINE`/`ONLINE` text with red/green status dots, shared LED/display/serial state output, bounded capture handoff, local service exchange, response playback, recoverable error states, and host-side flow coverage. | Run one complete exchange and an offline-to-online reconnect on physical hardware, recording the red/green LCD indicator, button behavior, capture, upload, response text, playback, and serial diagnostics. |
