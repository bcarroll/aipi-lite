...
1. ✅ Complete | 🟡 Not Validated - `feat/01-backup-recovery` (retired Unix
   workflow; superseded by the Windows CMD installer)
...
- ✅ Complete | ✅ Validated - `tooling/dev-install-capture` (retired Unix
  workflow; superseded by `dev_install.cmd`)
...
| `tooling/dev-install-capture` | Retired Unix workflow (historical) | The now-removed `dev_install.sh` and `install.sh --trace`, plus their deleted tests `tests/test_dev_install_capture.py` / `tests/test_install_script.py`, once added host-only installer capture and trace diagnostics (raw/redacted transcripts, run metadata, hardware validation notes, GitHub-ready issue bodies, automatic issue creation, installer phase transitions, firmware metadata, upload inventory, and command status under ignored local storage). | Retired. Use the Windows `dev_install.cmd` capture wrapper (backed by `tools/windows_installer.py`) for developer captures and validation runs. |
| `tooling/windows-device-validation` | Implemented, validation workflow physically exercised | `install.cmd`, `dev_install.cmd`, `validate.cmd`, `tools/device_application.py`, `tools/windows_installer.py`, host tests, `README.md`, and `tools/README.md` add cache-free application-first Windows uploads to the root-stat-safe `mpremote` `:/.` device-root destination, MicroPython firmware flashing via `install.cmd --flash-micropython` writing the `ESP32_GENERIC_S3-SPIRAM_OCT` build (required by the board's 8 MB Octal PSRAM; the plain build fails with `Wifi Out of Memory`), shared guarded cleanup of recognized misplaced `/src` trees and legacy root modules moved under `/lib`, cleanup/reset in one `mpremote` connection, a successful manual-power-cycle fallback when only reset confirmation fails, direct-install COM-port persistence in the ignored root `.conf` with safe explicit/saved/exactly-one selection, and a preflight hard reset with a five-second wait and same-port reconnect before validation and post-flash uploads, including the no-reset physical validation run for display, GPIO status/button, codec, capture, playback, local Wi-Fi/health, and offline inference probes. The validation probes share one raw-REPL session, report each probe result, and do not reconnect between probes; Wi-Fi is an informational optional probe, and `pass` plus `not-observed` are accepted evidence while explicit operator failures and required, structural, or transport failures remain nonzero. Each parsed run preserves redacted local and shareable validation evidence, including SSID-free `wifi_trace` diagnostics, and includes up to 12 redacted high-signal upload diagnostics on an application-upload failure before creating a new GitHub issue when `gh` is available. [GitHub issue #41](https://github.com/bcarroll/aipi-lite/issues/41) records a 2026-07-29 physical run with aggregate, upload, and batch status `0`; every required probe returned `0`, the informational Wi-Fi probe returned `1` with `no_ap_found`, display/LED/button observations passed, microphone/speaker/inference UI observations were `not-observed`, and the shared evidence retained SSID-free Wi-Fi diagnostics. | Repeat with a reachable local Wi-Fi network and service, and observe microphone acoustic input, speaker audibility, and inference UI responsiveness. |
...
| `spike/13-on-device-inference-feasibility` | Implemented, hardware validation pending | `src/lib/inference_probe.py`, `INFERENCE_FEASIBILITY.md`, `README.md`, `src/README.md`, `dev_install.cmd`, `tools/windows_installer.py`, and host tests add an offline-first simulated inference resource probe, deterministic local prompt fixture, model metadata validation, no-network policy checks, a feasibility report template, and redacted GitHub-ready bench capture on Windows. (The retired Unix `dev_install.sh` capture path has been removed.) | Run the `dev_install.cmd --inference-probe` capture on physical hardware and record heap, flash, timing, button, LED, display, and decision behavior. Speaker output is not required for this validation path. |
...
- 2026-07-29: [GitHub issue
  #41](https://github.com/bcarroll/aipi-lite/issues/41) recorded a complete
  `validate.cmd` workflow with aggregate, application-upload, and batch status
  `0`. Display, I/O, codec, capture, playback, and inference probes returned
  `0`; informational Wi-Fi returned `1` after `no_ap_found`. Display, status
  LED, and button observations passed. Microphone, speaker, and inference UI
  observations were `not-observed`, so acoustic behavior, inference UI
  responsiveness, and reachable local Wi-Fi/service behavior remain pending.
  The GitHub-ready evidence included SSID-free `wifi_trace` diagnostics.
- 2026-07-28: Roadmap update. The Windows CMD scripts (`install.cmd`,
  `dev_install.cmd`, `validate.cmd`, backed by `tools/windows_installer.py`) are
  the supported installer surface; the Unix `install.sh` / `dev_install.sh`
  scripts and their tests have been removed. `install.cmd --flash-micropython`
  now flashes the Octal-SPIRAM build `ESP32_GENERIC_S3-SPIRAM_OCT`, required
  because the board has 8 MB Octal PSRAM; the plain `ESP32_GENERIC_S3` build
  leaves the ESP-IDF Wi-Fi driver without internal DRAM and fails instantly with
  `Wifi Out of Memory`. Automated stock-firmware backup and restore remain
  unavailable through repository scripts and are manual `esptool` recovery steps.
...
> **Retired Unix workflow (historical):** the `install.sh` automation described
> in this branch has been removed. MicroPython flashing is now
> `install.cmd --flash-micropython` (SPIRAM_OCT build); stock backup and restore
> are manual `esptool` recovery steps. The design record below is kept as history.
...
> **Retired Unix workflow (historical):** the `dev_install.sh` / `install.sh`
> wrappers described in this branch have been removed. The supported developer
> capture surface is now `dev_install.cmd`, backed by
> `tools/windows_installer.py`. The design record below is kept as history.
...
- The shared Wi-Fi connector emits always-on, serial-only `wifi_trace` lifecycle
  events during normal boot, reconnects, and explicit probes. Status changes are
...
- `dev_install.cmd` can run the offline probe after an application-only,
  no-reset upload and independently create a redacted GitHub issue when the
  connected host explicitly supplies `--gh` plus an authenticated GitHub CLI.
  Failed GitHub publishing leaves the issue body in ignored local tooling
  without changing the measured validation result. (The earlier Unix
  `dev_install.sh` capture path has been retired.)
