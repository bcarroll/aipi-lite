...
| `tooling/windows-device-validation` | Implemented, hardware validation pending | `validate.cmd`, `tools/windows_installer.py`, `tests/test_windows_installer.py`, `README.md`, and `tools/README.md` add a no-reset Windows physical validation run for display, GPIO status/button, codec, capture, playback, and offline inference probes. Each parsed run records explicit operator observations, preserves redacted local evidence, and creates a new GitHub issue when `gh` is available. | Run `validate.cmd --port COMx --yes` on the physical AIPI-Lite and review the created issue evidence. |
