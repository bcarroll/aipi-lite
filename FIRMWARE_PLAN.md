...
| ES8311 audio control and I2S audio | Codec control and microphone capture implemented; playback not implemented | `src/es8311.py` and `src/audio_probe.py` configure the ES8311 over I2C at expected address `0x18`, keep the DAC muted, and default GPIO9 speaker enable off. `src/audio_capture.py` and `src/capture_probe.py` add bounded 16 kHz 16-bit mono capture, WAV packaging, and serial level metrics. Speaker playback remains a later milestone. |
